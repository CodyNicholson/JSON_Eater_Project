package com.jsoneater.error;

import com.jsoneater.constant.Constant;
import com.jsoneater.error.restCustomExceptions.*;
...
public class ControllerAdviceExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleHttpMessageNotReadable(
        ...
        logger.error(Constant.REST_BAD_REQUEST, ex);

        ErrorResponseModel errorDetails = new ErrorResponseModel(
                400,
                dateFormat.format(new Date()),
                Constant.REST_BAD_REQUEST,
                Constant.INVALID_JSON);
    ...
    protected ResponseEntity<ErrorResponseModel> handleUnauthorizedException(Exception ex) {
        logger.error(Constant.REST_UNAUTHORIZED, ex);

        ErrorResponseModel errorDetails = new ErrorResponseModel(
                401,
                dateFormat.format(new Date()),
                Constant.REST_UNAUTHORIZED,
                ex.getMessage());
    ...
    protected ResponseEntity<ErrorResponseModel> handleForbiddenException(Exception ex) {
        logger.error(Constant.REST_FORBIDDEN, ex);

        ErrorResponseModel errorDetails = new ErrorResponseModel(
                403,
                dateFormat.format(new Date()),
                Constant.REST_FORBIDDEN,
                ex.getMessage());
    ...
    protected ResponseEntity<ErrorResponseModel> handleNotFoundException(Exception ex) {
        logger.error(Constant.REST_NOT_FOUND, ex);

        ErrorResponseModel errorDetails = new ErrorResponseModel(
                404,
                dateFormat.format(new Date()),
                Constant.REST_NOT_FOUND,
                ex.getMessage());
    ...
    protected ResponseEntity<ErrorResponseModel> handleConflictException(Exception ex) {
        logger.error(Constant.REST_CONFLICT, ex);

        ErrorResponseModel errorDetails = new ErrorResponseModel(
                409,
                dateFormat.format(new Date()),
                Constant.REST_CONFLICT,
                ex.getMessage());
    ...
    protected ResponseEntity<ErrorResponseModel> handleInternalServerError(Exception ex) {
        logger.info(Constant.REST_INTERNAL_SERVER_ERROR, ex);

        ErrorResponseModel errorDetails = new ErrorResponseModel(
                500,
                dateFormat.format(new Date()),
                Constant.REST_INTERNAL_SERVER_ERROR,
                ex.getMessage());
    ...
    protected ResponseEntity<ErrorResponseModel> handleServiceUnavailableError(Exception ex) {
        logger.info(Constant.REST_SERVICE_UNAVAILABLE, ex);

        ErrorResponseModel errorDetails = new ErrorResponseModel(
                503,
                dateFormat.format(new Date()),
                Constant.REST_SERVICE_UNAVAILABLE,
                ex.getMessage());
